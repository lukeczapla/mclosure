...
int main(int argc, char **argv) {
  if (argc != 5) {
    printf("usage: translate_protein [step.dat] [step2.dat] [input.pdb] [output.pdb]\n");
    exit(0);
  }
  ...
  read_reference(argv[2], &W2);

  matrix n = identity(4);

  writematrix(stdout, W);

  rewrite_pdb(argv[3], argv[4], invert(W), W2); 
