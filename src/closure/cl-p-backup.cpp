...
//		      printf("Q=%d (%d %d %d)  (pairs %d, %d)\n", Q, post[Q-1], typet[Q-1], directt[Q-1], elementID, i);
	          long *lls = new long[WA[elementID].nP+nPs[Q]];
	          ...
	          for (int i = 0; i < nPs[Q]+WA[elementID].nP-1; i++) {
			    posh->add_data((lls[i+1]/(2*nstructs))-(lls[i]/(2*nstructs)));
              } 
//	          printf("\n");
		}
	      }
