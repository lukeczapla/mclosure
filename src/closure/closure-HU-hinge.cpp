...
void closure::reducedcombination() {
  ...
  int nZ = (int)(((FLOAT)(nsteps)*3.4+160.0)/epsilon)+2;
  int nY = (int)(((FLOAT)(nsteps)*3.4+160.0)/epsilon)+2;
  int nX = (int)(((FLOAT)(nsteps)*3.4+160.0)/epsilon)+2;
  ...
  for (i = 0; i < N; i++) {
    ...
    for (int Q = 0; Q <= 22; Q++) {
      if ((Q < 14) && (Q != 0)) place_HU(b, nsteps/2-Q);
      locs[Q] = place_HU(b, ((Q != 0) && (Q < 14) ? nsteps/2-Q+14 : nsteps/2), nsteps-14, (Q != 0 ? nsteps/2-Q : 0), nhus[Q]);
    ...
    for (int q = 0; q < nbounds; q++) {
      ...
      for (int o = (Xindex-dis < 0 ? 0: Xindex-dis); o <= (Xindex+dis > nX-1 ? nX-1 : Xindex+dis); o++) {
		...
		if ((strlen(spacingH) > 0) && (i != 0)) spacing[q].add_data((FLOAT)(lls[i]-lls[i-1]));
		  ...
		  tpt = calculatetp(calculateW(W)*WM*invert(postW));
		  rad->add_data(sqrt(tpt(4,1)*tpt(4,1)+tpt(5,1)*tpt(5,1)+tpt(6,1)*tpt(6,1)));
  ...
  char s[40];
  sprintf(s, "radial-hinge-HU-%d", nsteps);
