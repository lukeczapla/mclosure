...
//			  writematrix(stdout, Wi);
			...
			if (nPtotal == 2) {
			  ...
			  N2++;
				...
				if ((make_pics) || ((make_pics_sc) && (fabs(Lk0-writhe-twist) > 0.01))) {
					char s1[40], s2[40];
					sprintf(s1, "structure-%dbp-ID%d-B%d-%d.dat", nsteps, key, q, Ntw[q]);
					sprintf(s2, "proteins-%dbp-ID%d-B%d-%d.pdb", nsteps, key, q, Ntw[q]);
  ...
  for (int i = 0; i < nbounds; i++) {
    char s[50];
    if (Ntw[i] > 0) {
      sprintf(s, "writhe_distribution-%dbp-ID%d-B%d", nsteps, key, i);
      hwrithe[i]->printhistogram_mm(s);
      sprintf(s, "twist_distribution-%dbp-ID%d-B%d", nsteps, key, i);
      htwist[i]->printhistogram_pm(s);
      sprintf(s, "link_distribution-%dbp-ID%d-B%d", nsteps, key, i);
      hlink[i]->printhistogram_pi(s);
      sprintf(s, "R_g_distribution-%dbp-ID%d-B%d", nsteps, key, i);
      hRg[i]->printhistogram_pm(s);

      char posf[60];
      sprintf(posf, "protein_distances-%dbp-ID%d-B%d", nsteps, key, i);
      posh[i]->printhistogram_pi(posf, 2);
      sprintf(posf, "protein_positions-%dbp-ID%d-B%d", nsteps, key, i);
      posh2[i]->printhistogram_pi(posf, 1);
      sprintf(posf, "protein_number-%dbp-ID%d-B%d", nsteps, key, i);
