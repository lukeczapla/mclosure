...
void closure::reducedcombination() {
  ...
  init_positions(nsteps/2, nsteps-14);

  matrix W1H;

  histogram *rad = new histogram(340, 10.0);
  ...
  do {
    ...
    locs = place_proteins(this, nsteps/2, nsteps-14, nsteps/2, nsteps-14, 0, mnP);

    for (int j = nsteps/2; j < nsteps; j++) W1H = W1H*calculateW(v[j]);
    FLOAT r2 = sqrt(W1H(1,4)*W1H(1,4)+W1H(2,4)*W1H(2,4)+W1H(3,4)*W1H(3,4));
    if (r2 > 0.4*3.4*500.0) {
    i++;
	for (int j = 0; j < N; j++) {
	  matrix WTT = W1H*calculateW(WA[j].W);
      FLOAT rT;
      rad->add_data(rT = sqrt(WTT(1,4)*WTT(1,4)+WTT(2,4)*WTT(2,4)+WTT(3,4)*WTT(3,4)));
	  if (rT > 0.8*1000.0*3.4) {
	    printf("configuration found (r = %lf): ", rT);
	    for (int gg = 0; gg < WA[j].nP; gg++)
		  printf("%d ", WA[j].locs[gg]/(2*nstructs));
        for (int gg = 0; gg < mnP; gg++)
		  printf("%d ", locs[gg]/(2*nstructs));
	    printf("\n");
      }
    }
	}
  ...
  printf("%d out of %d second halves\n", N, Nt2);

  rad->printhistogram_pi("radial-HU-long");
...
//    sortlocs(WA[i].locs, WA[i].nP);
    ...
    W = endth;
    WA[i].W = W;
    if (sqrt(W(4,1)*W(4,1)+W(5,1)*W(5,1)+W(6,1)*W(6,1)) > 3.4*500.0*0.4) i++;

  } while (i < N);

  printf("%d out of %d sampled are greater than 0.4L\n", N, Nt1);
