...
main(int argc, char **argv) {
  if (argc != 3) {
    printf("usage: translate_startend [start.dat] [end.dat]\n");
    exit(0);
  }

  matrix Ws(4,4);
  matrix We(4,4);
  read_reference(argv[1], &Ws);
  read_reference(argv[2], &We);

  matrix n = invert(Ws);

  writematrix(stdout, n*We);
  writematrix(stdout, calculatetp(n*We));
